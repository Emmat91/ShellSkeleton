...
static int CopyWord(char *token, struct CommandData *data, int *state)
{
    switch (*state) {
        case COMMAND: 
             ...
             if (strstr(data->TheCommands[data->numcommands].command,"cd")!= NULL 
              || strstr(data->TheCommands[data->numcommands].command,"exit")!= NULL
              || strstr(data->TheCommands[data->numcommands].command,"export")!= NULL 
              || strstr(data->TheCommands[data->numcommands].command,"set") != NULL
              || strstr(data->TheCommands[data->numcommands].command,"pwd") != NULL)
              {
                data->incommand = 1;
              }
        ...
        case ASSIGN:
             if (data->value != NULL) {
                 fprintf(stderr,"Error, value is assigned twice\n");
                 return 0;
             }
...
void Debugging(char *line, struct CommandData *data)
{
    printf("DEBUG OUTPUT:\n---------------------------\n");
    ...
    if(data->infile == NULL)
      printf("DEBUG:  Input file  : (NULL)\n");
    else
      printf("DEBUG:  Input file  : %s\n", data->infile);

    if(data->outfile == NULL)
      printf("DEBUG:  Output file : (NULL)\n");
    else
      printf("DEBUG:  Output file : %s\n", data->outfile);
    
    if (data->background == 0)
      printf("DEBUG:  Background option : OFF\n");
    else
      printf("DEBUG:  Background option : ON\n");

    if (data->incommand==1)
    {
      printf("DEBUG:  Build-in command : YES\n");
    }
    else
      printf("Build-in command : NO\n");
    printf("---------------------------\n");
}

extern void Execute(struct CommandData data)
{
